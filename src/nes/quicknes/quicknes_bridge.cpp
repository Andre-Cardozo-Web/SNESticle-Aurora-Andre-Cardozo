...
{
    const Uint8 *rom = (const Uint8 *)pData;
    if (!rom || nBytes < 16 || rom[0] != 'N' || rom[1] != 'E' || rom[2] != 'S' || rom[3] != 0x1A) return -1;
    if ((rom[7] & 0x0CU) != 0x08U) return -1;
...
static bool qBattleBoxCrc(Uint32 crc) {
    if (crc == 0x61A852EAU || crc == 0x78B657ACU || crc == 0x803B9979U || crc == 0xC22BC87BU) return true;
    return false;
}

static bool qFamicomLightGunCrc(Uint32 crc) {
    if (crc == 0x0AFB395EU || crc == 0x24598791U || crc == 0x2A6559A1U) return true;
    return false;
}

static bool qTwoNesZapperCrc(Uint32 crc) {
    if (crc == 0x231BC76EU || crc == 0xB79F2651U || crc == 0xD15009CCU) return true;
    return false;
}

static bool qNesZapperCrc(Uint32 crc) {
    if (crc == 0x01B87025U || crc == 0x04A6B46DU || crc == 0x051E60C6U) return true;
    return false;
}
...
static void qUpdateLightGunAim(Emu::SysInputT *pInput) {
    ...
    if (InputIsPadConnected(0)) {
        Uint32 packed = InputGetPadAnalog(0);
        ax = (packed >> 16) & 0xFFU; ay = (packed >> 24) & 0xFFU;
    ...
    if (s_GunX < 1536) s_GunX = 1536; if (s_GunX > 63744) s_GunX = 63744;
    if (s_GunY < 1536) s_GunY = 1536; if (s_GunY > 59648) s_GunY = 59648;
...
static void qUpdateArkanoidVaus(Emu::SysInputT *pInput) {
    unsigned axis = 0x80U; if (!s_ArkanoidVaus) return;
    if (InputIsPadConnected(0)) axis = (InputGetPadAnalog(0) >> 16) & 0xFFU;
...
static void qResetTransient(void) {
    memset(s_Video, 0, sizeof(s_Video)); memset(s_LastFramePalette, 0, sizeof(s_LastFramePalette));
    memset(s_Pending, 0, sizeof(s_Pending)); s_PendingCount = 0; s_PaletteValid = false;
    qResetDirectVideo(); s_TurboPhase = false; s_TurboFrame = s_TurboSpeedShift = 0;
    s_SkipVideoNext = false; s_LastSpriteScanlineLimit = s_LastSpriteScreenLimit = -1;
}

static Uint8 qMapPad(Uint16 pad) {
    if (pad == EMUSYS_DEVICE_DISCONNECTED) return 0;
    Uint8 nes = 0;
    if (pad & SNESIO_JOY_B) nes |= 0x01; if (pad & SNESIO_JOY_Y) nes |= 0x02;
    if ((pad & SNESIO_JOY_A) && s_TurboPhase) nes |= 0x01; if ((pad & SNESIO_JOY_X) && s_TurboPhase) nes |= 0x02;
    if (pad & SNESIO_JOY_SELECT) nes |= 0x04; if (pad & SNESIO_JOY_START) nes |= 0x08;
    if (pad & SNESIO_JOY_UP) nes |= 0x10; if (pad & SNESIO_JOY_DOWN) nes |= 0x20;
    if (pad & SNESIO_JOY_LEFT) nes |= 0x40; if (pad & SNESIO_JOY_RIGHT) nes |= 0x80;
    return nes;
}
...
static void qRenderFrame(CRenderSurface *pTarget) {
    ...
    if (!s_PaletteValid || memcmp(s_LastFramePalette, frame.palette, sizeof(s_LastFramePalette)) != 0) {
        for (unsigned i = 0; i < 256; ++i) {
            ...
            Uint8 r, g, b; qGetRgb(ci >= (unsigned)Nes_Emu::color_table_size ? 0 : ci, &r, &g, &b);
            s_RgbaPalette[i] = 0xff000000u | ((Uint32)b << 16) | ((Uint32)g << 8) | r;
        }
        memcpy(s_LastFramePalette, frame.palette, sizeof(s_LastFramePalette)); s_PaletteValid = true;
    }
    for (int y = 0; y < Nes_Emu::image_height; ++y) {
        Uint32 *dst = (Uint32 *)pTarget->GetLinePtr(y);
        if (dst) { const Uint8 *src = frame.pixels + (long)y * frame.pitch; for (int x = 0; x < Nes_Emu::image_width; ++x) dst[x] = s_RgbaPalette[src[x]]; }
    }
}
...
static void qRefreshGsPalette(const Nes_Emu::frame_t &frame) {
    ...
    for (unsigned i = 0; i < 256; ++i) {
        unsigned ci = (unsigned)(unsigned short)frame.palette[i]; unsigned dest = i; unsigned block = i & 0x3fU;
        if ((block & 0x18U) == 0x08U) dest += 8U; else if ((block & 0x18U) == 0x10U) dest -= 8U;
        Uint8 r, g, b; qGetRgb(ci >= (unsigned)Nes_Emu::color_table_size ? 0 : ci, &r, &g, &b);
...
bool QuicknesBridge_DrawDirectGs(Uint32 auroraOutBaseTBP, Int32 logicalY, Float32 intensity) {
    ...
    if (s_DirectUploadSerial != s_DirectFrameSerial) {
        GPPrimUploadTexture((int)texTBP, QN_GS_T8_TBW, 0, 0, GS_PSMT8, frame.pixels, QN_VIDEO_W, Nes_Emu::image_height);
        s_DirectUploadSerial = s_DirectFrameSerial;
    }
    qRefreshGsPalette(frame);
    if (!s_DirectClutResident) { GPPrimUploadTexture((int)clutTBP, 64, 0, 0, GS_PSMCT32, s_GsPalette, 16, 16); s_DirectClutResident = true; }
    GPPrimSetTex(texTBP, QN_GS_T8_TBW, 9, 8, GS_PSMT8, clutTBP, 64, GS_PSMCT32, 0);
    Uint32 mod = (Uint32)(128.0f * intensity + 0.5f); if (mod > 128u) mod = 128u;
...
static void qDrainAudio(CMixBuffer *pMix) {
    ...
    if (s_PendingCount > 0) memcpy(s_AudioOut, s_Pending, (size_t)s_PendingCount * sizeof(s_Pending[0]));
    long count = s_pEmu->read_samples((short *)(s_AudioOut + s_PendingCount), QN_AUDIO_MAX);
    if (count <= 0) { pMix->Flush(); return; }
    int n = s_PendingCount + (int)(count > QN_AUDIO_MAX ? QN_AUDIO_MAX : count);
    int flush = n & ~3; s_PendingCount = n - flush;
    if (s_PendingCount > 0) memcpy(s_Pending, s_AudioOut + flush, (size_t)s_PendingCount * sizeof(s_Pending[0]));
    if (flush > 0) pMix->OutputSamplesMono(s_AudioOut, flush);
    pMix->Flush();
}

bool QuicknesBridge_Init(void) {
    if (s_Initialized) return true; s_pEmu = new Nes_Emu(); if (!s_pEmu) return false;
    s_pAudioBuffer = new Nes_Buffer(); if (!s_pAudioBuffer) { delete s_pEmu; return false; }
    if (s_pEmu->set_sample_rate(32000, s_pAudioBuffer)) { delete s_pAudioBuffer; delete s_pEmu; return false; }
    s_pEmu->set_palette_range(0); s_pEmu->set_sprite_mode(Nes_Emu::sprites_visible); s_pEmu->set_pixels(s_Video + 8, QN_VIDEO_W);
    quicknes_snesticle_set_duty_swap(s_DutySwap ? 1 : 0); qResetTransient(); s_Initialized = true; return true;
}
...
bool QuicknesBridge_LoadGame(const void *pData, size_t nBytes, const char *pName) {
    ...
    if (s_GameLoaded) QuicknesBridge_UnloadGame(); qResetTransient(); s_pEmu->set_pixels(s_Video + 8, QN_VIDEO_W);
    Mem_File_Reader reader(pData, (long)nBytes); if (s_pEmu->load_ines(reader)) { s_pEmu->close(); return false; }
    Uint32 payloadCrc = qNesPayloadCrc32(pData, nBytes); s_LightGunDetectedMode = qLightGunModeForCrc(payloadCrc);
    s_LightGunMode = s_LightGunEnabled ? s_LightGunDetectedMode : 0; s_ArkanoidVaus = (payloadCrc == 0xD89E5A67U);
    quicknes_snesticle_ext_set_turbofile(0); quicknes_snesticle_ext_set_arkanoid(s_ArkanoidVaus ? 1 : 0);
    quicknes_snesticle_ext_set_battlebox(0); quicknes_snesticle_ext_set_lightgun(s_LightGunMode);
    quicknes_snesticle_ext_reset_bus(); s_GameLoaded = true; return true;
}

void QuicknesBridge_UnloadGame(void) {
    if (s_Initialized && s_pEmu) s_pEmu->close(); s_GameLoaded = s_ArkanoidVaus = s_TurboFileEnabled = s_BattleBoxEnabled = false;
    s_LightGunDetectedMode = s_LightGunMode = 0; quicknes_snesticle_ext_set_turbofile(0); quicknes_snesticle_ext_set_arkanoid(0);
    quicknes_snesticle_ext_set_battlebox(0); quicknes_snesticle_ext_set_lightgun(0); qResetLightGunAim(); qResetTransient();
}
