...
static void qUpdateLightGunAim(Emu::SysInputT *pInput) {
    ...
    bool trigger = false, offscreen = false;
    ...
    s_GunX += s_GunVX;
    s_GunY += s_GunVY;
    if (s_GunX < 1536) s_GunX = 1536;
    if (s_GunX > 63744) s_GunX = 63744;
    if (s_GunY < 1536) s_GunY = 1536;
    if (s_GunY > 59648) s_GunY = 59648;
...
bool QuicknesBridge_LoadGame(const void *pData, size_t nBytes, const char *pName) {
    ...
    if (s_pEmu->load_ines(reader)) {
        s_pEmu->close();
        return false;
    }
    Uint32 payloadCrc = qNesPayloadCrc32(pData, nBytes);
    s_LightGunDetectedMode = qLightGunModeForCrc(payloadCrc);
    s_LightGunMode = s_LightGunEnabled ? s_LightGunDetectedMode : 0;
...
void QuicknesBridge_UnloadGame(void) {
    ...
    s_GameLoaded = s_ArkanoidVaus = s_TurboFileEnabled = s_BattleBoxEnabled = false;
    s_LightGunDetectedMode = s_LightGunMode = 0;
    quicknes_snesticle_ext_set_turbofile(0);
    quicknes_snesticle_ext_set_arkanoid(0);
    quicknes_snesticle_ext_set_battlebox(0);
    quicknes_snesticle_ext_set_lightgun(0);
    qResetLightGunAim();
    qResetTransient();
}
