...
static Uint32 qNesPayloadCrc32(const void *pData, size_t nBytes) {
    ...
    if (!rom || nBytes < 16 || rom != 'N' || rom != 'E' || rom != 'S' || rom != 0x1A) return 0;
...
void QuicknesBridge_RunFrame(Emu::SysInputT *pInput, CRenderSurface *pTarget, CMixBuffer *pMixBuf) {
    if (!s_GameLoaded || !s_pEmu) return; s_TurboPhase = (((s_TurboFrame >> s_TurboSpeedShift) & 1U) == 0U); ++s_TurboFrame; 
    if (pInput && (pInput->uPad & SNESIO_JOY_L)) { quicknes_snesticle_set_microphone(1); } else { quicknes_snesticle_set_microphone(0); }
